...
export function initHeroFloating() {
  ...
  const tick = (t) => {
    ...
    state.forEach((s, i) => {
      ...
      // Parallax — extra-conservative now that items are +20% larger,
      // so max mouse-driven movement stays clear of neighboring floats
      const parX = mouse.x * s.depth * 16;
      const parY = mouse.y * s.depth * 12;
